...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        if not PYEXIV2_AVAILABLE:
            self.stdout.write(
                self.style.WARNING(
                    "pyexiv2 not available. EXIF data will not be extracted."
                )
            )
            ...
            try:
                with pyexiv2.Image(str(photo_path)) as img:
                    ...
                    if "Exif.Photo.DateTimeOriginal" in exif:
                        self.stdout.write(
                            f"  Date taken: {exif['Exif.Photo.DateTimeOriginal']}"
                        )
    ...
    def _extract_exif_data(self, photo_path: Path) -> dict[str, Any]:
        ...
        try:
            with pyexiv2.Image(str(photo_path)) as img:
                ...
                if "Exif.Photo.DateTimeOriginal" in exif:
                    try:
                        ...
                        exif_data["date_prise"] = datetime.strptime(
            ...
            self.stdout.write(
                f"Warning: EXIF extraction failed for {photo_path.name}: {e}"
            )
    ...
    def _import_photo(self, photo_path: Path, galerie: Galerie) -> None:
        ...
        with open(photo_path, "rb") as f:
            ...
            photo_version.fichier_web.save(photo_path.name, django_file, save=False)
