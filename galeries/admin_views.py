...
def upload_photos_view(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if not galerie_id or not uploaded_files:
            messages.error(
                request, "Veuillez sélectionner une galerie et au moins une photo."
            )
            ...
            try:
                with transaction.atomic():
                    ...
                    if not uploaded_file.content_type.startswith("image/"):
                        errors.append(
                            f"{uploaded_file.name}: Type de fichier non supporté"
                        )
                        continue
                    ...
                    with open(temp_path, "rb") as f:
                        ...
                        photo_version.fichier_web.save(
                            uploaded_file.name, django_file, save=False
                        )
