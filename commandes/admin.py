...
class ClientAdmin(admin.ModelAdmin):
    list_display = ['nom', 'prenom', 'email', 'entreprise', 'nombre_commandes', 'derniere_commande', 'cree_le']
    ...
    fieldsets = (
        ...
        ('Informations de contact', {
            'fields': ('telephone', 'entreprise', 'adresse')
        }),
        ('Notes internes', {
            'fields': ('notes',),
            'classes': ('collapse',)
...
class PhotoCommandeInline(SortableInlineAdminMixin, admin.TabularInline):
    model = PhotoCommande
    extra = 0
    fields = ['photo_originale', 'version_selectionnee', 'titre_personnalise', 'apercu_photo', 'ordre_affichage']
    ...
    def get_queryset(self, request: HttpRequest) -> models.QuerySet[PhotoCommande]:
        return super().get_queryset(request).select_related(
            'photo_originale', 'version_selectionnee', 'commande'
        )
...
class CommandeAdmin(SortableAdminMixin, admin.ModelAdmin):
    form = CommandeForm
    list_display = [
        'reference', 'titre', 'client', 'statut', 'nombre_photos', 'nombre_vues', 
        'est_accessible_display', 'expire_le', 'cree_le', 'actions_rapides'
    ]
    list_filter = ['statut', 'autoriser_telechargement_hd', 'cree_le', 'expire_le']
    search_fields = ['reference', 'titre', 'client__nom', 'client__prenom', 'client__email']
    readonly_fields = [
        'code_acces', 'reference', 'nombre_vues', 'premiere_visite_le', 
        'derniere_visite_le', 'nombre_telechargements', 'cree_le', 'modifie_le',
        'url_acces_client'
    ]
    ...
    fieldsets = (
        (None, {
            'fields': ('client', 'reference', 'titre', 'description')
        }),
    ...
    def get_queryset(self, request: HttpRequest) -> models.QuerySet[Commande]:
        return super().get_queryset(request).select_related('client').prefetch_related('photos')


@admin.register(PhotoCommande)
class PhotoCommandeAdmin(admin.ModelAdmin):
    list_display = ['commande', 'photo_originale', 'version_selectionnee', 'titre_personnalise', 'apercu_photo']
    list_filter = ['commande__statut', 'version_selectionnee__traitement', 'ajoutee_le']
    search_fields = [
        'commande__reference', 'commande__titre',
        'photo_originale__titre', 'titre_personnalise'
    ]
