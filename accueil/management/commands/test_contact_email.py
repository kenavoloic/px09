...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["send_real_email"]:
            ...
            self.stdout.write(
                "   Mode: SIMULATION (ajoutez --send-real-email pour un vrai envoi)"
            )
            self.stdout.write(
                "   ℹ️  Le formulaire utilise le backend: console en développement"
            )
        ...
        if "console" in settings.EMAIL_BACKEND.lower():
            self.stdout.write(
                self.style.WARNING("   ⚠️  Backend console actif (développement)")
            )
